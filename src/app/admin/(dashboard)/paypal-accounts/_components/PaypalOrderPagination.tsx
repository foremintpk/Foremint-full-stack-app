...
interface PaypalOrderPaginationProps {
  totalCount: number;
  totalPages: number;
  currentPage: number;
  pageSize: number;
...
export function PaypalOrderPagination({
  totalCount,
  totalPages,
  currentPage,
  pageSize,
}: PaypalOrderPaginationProps) {
  ...
  const updatePageParam = (pageNumber: number): string => {
    ...
    return `/admin/paypal-accounts?${params.toString()}`;
  };
  ...
  return (
    <nav
      aria-label="PayPal orders list pagination"
