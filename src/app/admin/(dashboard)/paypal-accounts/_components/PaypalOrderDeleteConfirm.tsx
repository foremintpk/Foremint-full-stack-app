...
import { deletePaypalOrder } from '@/lib/admin/actions/paypalOrderActions';
import { PaypalOrder } from '@/types/admin';

interface PaypalOrderDeleteConfirmProps {
  order: PaypalOrder;
  onClose: () => void;
}

export function PaypalOrderDeleteConfirm({
  order,
  onClose,
}: PaypalOrderDeleteConfirmProps) {
  ...
  const handleDelete = () => {
    ...
    startTransition(async () => {
      const res = await deletePaypalOrder(order.id);
      if (res && res.error) {
        setError(res.error);
      } else {
        onClose();
    ...
    >
      ...
      >
        ...
        <div className="bg-red-50 px-6 py-4 text-red-700 rounded-t-2xl">
          <h3 className="text-lg font-bold font-manrope">Delete Order</h3>
        </div>

        {/* Content */}
        <div className="p-6">
          <p className="text-sm text-gray-600 font-inter">
            This permanently deletes the order for{' '}
            <strong className="text-gray-900 font-semibold">{order.customerName}</strong>.
